...
def run_doc2vec(train_docs, test_docs, alldocs, dm, size, window, alpha, negative, sample, cores, min_count, passes, output):
    ...
    model = Doc2Vec(dm=dm, size=size, window=window, alpha = alpha, negative=negative, sample=sample, workers=cores, min_count = min_count, iter = 1)
    ...
    with elapsed_timer() as elapsed:
        ...
        for epoch in range(passes):
            ...
            N = 1000
            cost_function(model, train_docs, N)
            cost_function(model, train_docs, len(train_docs))
        duration = '%.1f' % elapsed()

        whole_duration += elapsed()
        
        for i, doc in enumerate(test_docs):
            infer_vecs[i, :] = model.infer_vector(doc.words, alpha=0.1, min_alpha=0.0001, steps=5)
            #model.docvecs['SENT_' + str(len(train_docs) + i)] = infer_vecs[i, :]    
            test_vectors[i] =  tuple([infer_vecs[i, :], doc.tags])
        cost(model, test_vectors, test_docs, len(test_docs))
