...
def main(space_dir, classifier, C = None):
    ...
    """ Write evaluation results to the Res_concat_20ng.csv DataFrame in separate directories and to the output"""
                ...
                for other_model in os.listdir(space_dir): #for every model in the vectors directory
                    if other_model.endswith('.txt'):      #if it is the name of saved model
                        if ('cbow 1' in other_model):     #if it is a PV-DM model
                            ...
                            try:
                                DocumentVectors0_0, train_labels, DocumentVectors1_0, test_labels = DocumentVectors(space_dir + model)
                            except:
                            ...
                            try:
                                DocumentVectors0_1, train_labels, DocumentVectors1_1, test_labels = DocumentVectors(space_dir + other_model+'.txt')
                            except:
                            ...
                            run_dir = './runs_20ng/%s-%s/' % (dir_name, time_str())#and after starting time
                            ...
                            df.to_csv(run_dir + "Res_concat_20ng" + classifier + ".csv")
                            #and to the output
                            print (other_model)
                            print (model)
                            print (accuracy)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Input: vector directory, classifier name, regularization parameter (optional). Writes to stdout the best classifier accuracy, precision, recall, F-score and in case of no given C parameter - cross validation accuracy from GridSearch.')
    
    parser.add_argument("-vectors", nargs='?', default='space_p2v_20ng/', help = 'paragraph2vec vectors file.')
